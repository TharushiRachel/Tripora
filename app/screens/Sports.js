...
const Sports = ({navigation, onPress,clicked, searchPhrase, setSearchPhrase}) => {
    const [items,setArticles] = useState([]);
    const getNews = () => {
        axios.get('https://api.rss2json.com/v1/api.json?rss_url=https%3A%2F%2Fwww.newsfirst.lk%2Fcategory%2Fsports%2Ffeed%2F')
            .then( (response) =>{
                // handle success
