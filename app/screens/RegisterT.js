...
export default class RegisterT extends React.Component {
  ...
  signUp = async () => {
    const { username, password, email, phone, fullname, licenseNo, languageProficiencies, type } = this.state
    axios
  ...
  onSubmit(e){
    ...
    axios.post('http://localhost:3000/tour-guide', guide)
.then(response => {
    ...
    return (
      <View style={styles.container}>
        ...
        <TextInput
          ...
          autoCapitalize="none"
          placeholderTextColor='#B5B5B5'
          onChangeText={val => this.onChangeText('languageProficiencies', val)}
        // value={this.state.phone}
        // onChange={this.onChange}
        />
        <TextInput
